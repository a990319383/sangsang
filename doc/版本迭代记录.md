...
- 【<font color='skyblue'>**调整**</font>】项目名字已经修改

  > 旧名字语义渐渐不适用于本框架定位，且无法快速搜索定位到本项目，于是改名

  - 项目名字变化

    【field-encryptor】 ==>【 sangsang】

  - 最新git地址

    ```
    https://gitee.com/tired-of-the-water/sangsang
    ```
...
- 【<font color='skyblue'>**调整**</font>】核心配置的开头已经修改

  > 为保证使用者能快速找到框架，并获取到使用文档，修改核心配置的前缀
  ...
  - 修改后
    ...
    ```properties
    sangsang.scanEntityPackage
    sangsang.lruCapacity
    ...
    ```
...
- 【<font color='orange'>**新增**</font>】fieldparse增加了对CTE的支持

- 【<font color='skyblue'>**调整**</font>】修改全局对大小写敏感的判断，解决某些Local环境下，某些字母小写转换和程序不一致问题

  > 旧：xxx.toLowerCase()
  >
  > 新：xxx.toLowerCase(Locale.ROOT)

- 【<font color='skyblue'>**调整**</font>】读取当前数据源的版本信息调整

  > TableCache.getDataSourceConfig() 缓存的当前DataSource的版本号信息做出调整
  >
  > - 旧逻辑：取第一个DataSource的版本号信息
  > - 新逻辑：只有当所有的DataSource的版本号信息一致时才进行缓存，异构多数据源时不进行缓存
...
- 【<font color='orange'>**新增**</font>】pojo模式支持xml中返回值配置resultMap，支持property和column变量名完全不同的场景

- 【<font color='skyblue'>**调整**</font>】pojo模式下，xml中sql的字段到java类的映射方式调整

  > - 旧逻辑：写死的下划线自动转驼峰进行映射，不区分大小写
  > - 新逻辑：根据项目mybatis的配置来决定是否进行下划线到驼峰的转换映射

- 【<font color='orange'>**新增**</font>】pojo模式查询结果解密时支持批量解密

  > ```
  > FieldEncryptorStrategy接口新增 batchDecryption(List<String> datas) 方法
  > pojo模式下覆写这个方法即可支持批量解密，可在使用密码机解密的场景下大幅度优化性能
  > 如果不覆写的话，默认循环调用之前旧的单个解密接口
  > ```

#### 异构数据库语法转换

- 【<font color='tomato'>**bug修复**</font>】对于oracle数据库的 || 这种连接语法提供支持，oracle2mysql增加此类语法的兼容


#### 数据权限隔离

- 【<font color='tomato'>**bug修复**</font>】开启DML数据权限隔离时，insert select 语法兼容

  > 旧版本只支持 insert (select ) 这种带括号的，修复后带不带括号都支持
  >
  > 详见com.sangsang.test.IsolationTest#i3

- 【<font color='tomato'>**bug修复**</font>】开启DML数据权限隔离时 update语句中存在from语法兼容

  > 详见com.sangsang.test.IsolationTest#u4

- 【<font color='tomato'>**bug修复**</font>】增加一些较为冷门的语法支持

  > 详见com.sangsang.test.IsolationTest#s9 到 s15

- 【<span style="color: #EE82EE;">**bug修复**</span>】在caseSensitive=true 时原sql存在大写 OR时，会导致拼接的权限隔离 and or的关系错误
