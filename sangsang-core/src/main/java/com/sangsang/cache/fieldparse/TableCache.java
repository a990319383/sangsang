...
public class TableCache extends DefaultBeanPostProcessor {
     ...
     * 缓存当前的配置信息
     */
    private static SangSangProperties sangSangProperties;
    /**
    ...
    public static void init(List<DataSource> dataSources, SangSangProperties sangSangProperties) {
        ...
        //2.扫描配置的路径校验
        if (!sangSangProperties.isAutoFill() && com.sangsang.util.CollectionUtils.isEmpty(sangSangProperties.getScanEntityPackage())) {
            log.warn("【sangsang】当前未配置实体类扫描路径也未开启自动填充，如需使用字段脱敏以外的功能，请完善此配置");
            return;
        }

        //3.开始扫描配置路径的实体类
        List<TableInfoDto> tableInfoDtos = sangSangProperties.getScanEntityPackage().stream().map(m -> parseTableInfoByScanEntityPackage(m)).flatMap(Collection::stream).collect(Collectors.toList());
        log.info("【sangsang】初始化表结构信息，扫描指定包路径 :{} 合计表数量：{}", sangSangProperties.getScanEntityPackage(), tableInfoDtos.size());

        //4.将表结构信息处理，加载到缓存的各个Map中
        fillCacheMap(tableInfoDtos);

        //5.如果开启了表结构填充配置的话，则进行表结构填充
        if (sangSangProperties.isAutoFill()) {
            FieldFillUtil.fieldFill(dataSources, sangSangProperties);
        }

        //6.如果没有开启表结构自动填充的话，去掉不必要表的缓存，避免某些逻辑进行不必要的循环（如果开启了自动填充，填充里面会自动过滤掉不需要的缓存信息）
        if (!sangSangProperties.isAutoFill()) {
            simplificationCache(sangSangProperties);
        }
    ...
    private static void cacheDataSourceConfig(List<DataSource> dataSources) {
        ...
        if (CollectionUtils.isEmpty(dataSources)) {
            log.warn("【sangsang】当前Spring环境无可用的DataSource对象，某些信息无法自动加载，程序是否正常请二次验证");
            return;
        }
        ...
        if (CollectionUtils.isNotEmpty(identifierQuotes)) {
            sangSangProperties.setIdentifierQuote(new ArrayList<>(identifierQuotes));
        }
     ...
     * @Param []
     **/
    private static void simplificationCache(SangSangProperties sangSangProperties) {
        //1.如果开启了语法转换功能，则不进行精简，因为这个功能需要使用到整个库的全部表结构
        if (sangSangProperties.getTransformation() != null && StringUtils.isNotBlank(sangSangProperties.getTransformation().getPatternType())) {
            return;
        }

        //2.过滤只要我们功能中需要的表
        ((FieldHashMapWrapper) TABLE_FIELD_MAP).filter(f -> TableCache.getCurConfigTable().contains(f.getKey()));
        log.info("【sangsang】精简表结构信息，处理完毕 当前缓存表数量:{}张", TABLE_FIELD_MAP.keySet().size());
    }
     ...
     * @Param [sangSangProperties, dataSources]
     **/
    private static SangSangProperties fillIdentifierQuote(SangSangProperties sangSangProperties) {
        //1.如果手动指定了标识符的引用符，则返回
        if (CollectionUtils.isNotEmpty(sangSangProperties.getIdentifierQuote())) {
        ...
        if (sangSangProperties.getTransformation() != null
                && (TransformationPatternTypeConstant.MYSQL_2_DM.equals(sangSangProperties.getTransformation().getPatternType())
                || TransformationPatternTypeConstant.ORACLE_2_MYSQL.equals(sangSangProperties.getTransformation().getPatternType()))) {
            sangSangProperties.setIdentifierQuote(Arrays.asList(SymbolConstant.FLOAT, SymbolConstant.SINGLE_QUOTES));
            return sangSangProperties;
        }
        ...
        for (Class entityClass : entityClasses) {
            ...
            if (shardingTableEncryptor != null) {
                try {
                    ...
                    log.error("【sangsang】 @ShardingTableEncryptor 分表表名策略实例化失败，请确保策略有无参构造方法", e);
                }
            }
        }
        return result;
     ...
     * @Param []
     **/
    public static SangSangProperties getCurConfig() {
        return sangSangProperties;
    }
