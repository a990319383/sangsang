...
public class TransformationInterceptor implements Interceptor, BeanPostProcessor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        try {
            log.debug("【sangsang】<transformation>旧sql：{}", oldSql);
            ...
            if (StringUtils.isNotBlank(transformationStatementVisitor.getResultSql())) {
                ...
                log.debug("【sangsang】<transformation>新sql：{}", newSql);
            }
            ...
            log.error("【sangsang】<transformation>语法转换 sql异常 原sql:{}", oldSql, e);
        } finally {
    ...
    private void clearThreadLocal(String oldSql) {
        ...
        try {
            ...
            log.error("【sangsang】<transformation>移除TransformationHolder异常 原sql:{}", oldSql, e);
        }
        //手动清除，避免内存泄漏
        try {
            TfParameterMappingHolder.clearParameterMapping();
        } catch (Exception e) {
            log.error("【sangsang】<transformation>clearParameterMapping异常 原sql:{}", oldSql, e);
        }
        //手动清除，避免内存泄漏
        try {
            TfParameterMappingHolder.clearRemoveParameterMapping();
        } catch (Exception e) {
            log.error("【sangsang】<transformation>clearRemoveParameterMapping异常 原sql:{}", oldSql, e);
        }
    }
    ...
    public Object postProcessAfterInitialization(Object bean, String beanName) throws BeansException {
        ...
        if (SqlSessionFactory.class.isAssignableFrom(bean.getClass())) {
            ...
            if (sessionFactory.getConfiguration().getInterceptors().stream().filter(f -> TransformationInterceptor.class.isAssignableFrom(f.getClass())).findAny().orElse(null) == null) {
                ...
                log.info("【sangsang】<transformation>手动注册拦截器 TransformationInterceptor");
            }
